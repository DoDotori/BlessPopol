...
public class UILabel : UIWidget
{
	...
	void ProcessText (bool legacyMode, bool full)
	{
		...
		if (mPrintedSize > 0)
		{
			...
			for (int ps = mPrintedSize; ps > 0; --ps)
			{
				...
				// Wrap the text
				bool fits = NGUIText.WrapText(mText, out mProcessedText, true , this);
	...
	{
		if (UIProgressBar.current != null)
			text = Mathf.FloorToInt(UIProgressBar.current.value * 100f) + "%";
	}
	...
	public bool Wrap (string text, out string final, int height)
	{
		...
		bool retVal = NGUIText.WrapText(text, out final , this);
		NGUIText.bitmapFont = null;
#if DYNAMIC_FONT
		NGUIText.dynamicFont = null;
#endif
